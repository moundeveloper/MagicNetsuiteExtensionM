(()=>{const l={FOLDER_NAME:"MagicNetsuiteScripts",SERVER_FILE:"magic_netsuite_server.js",MCP_SERVER_FILE:"magic_netsuite_mcp_server.js",scriptId:"customscript_magic_netsuite_server",SUITELET_SCRIPT_NAME:"Magic Netsuite Server",SUITELET_SCRIPT_ID:"_magic_netsuite_server",SUITELET_DEPLOYMENT_NAME:"Magic Netsuite Server Deployment",SUITELET_SCRIPT_DEPLOY_ID:"_magic_netsuite_server_dp"};window.checkMagicNetsuiteComponents=async({query:e})=>{try{console.log("Checking server components...");const[t]=(await e.runSuiteQL.promise({query:"SELECT id FROM MediaItemFolder WHERE name = ? AND parent = -15",params:[l.FOLDER_NAME]})).asMappedResults();if(!t)return{folderExists:!1,serverFileExists:!1,suiteletScriptExists:!1,suiteletDeployed:!1,allReady:!1};const s=t.id,n=(await e.runSuiteQL.promise({query:"SELECT name FROM file WHERE folder = ? AND name IN (?, ?)",params:[s,l.SERVER_FILE,l.MCP_SERVER_FILE]})).asMappedResults(),o=new Set(n.map(c=>c.name)),a=o.has(l.SERVER_FILE),i=o.has(l.MCP_SERVER_FILE),[d]=(await e.runSuiteQL.promise({query:"SELECT id FROM script WHERE scriptid = ?",params:[l.scriptId]})).asMappedResults(),r=!!d;let u=!1;return r&&(u=(await e.runSuiteQL.promise({query:"SELECT primarykey AS id FROM scriptdeployment WHERE script = ?",params:[d.id]})).asMappedResults().length>0),{folderExists:!0,serverFileExists:a,mcpServerFileExists:i,suiteletScriptExists:r,suiteletDeployed:u,allReady:a&&i&&r&&u}}catch(t){return console.error("[CHECK_SERVER_COMPONENTS] Error:",t),{error:t.message}}};window.runQuickScriptServer=async(e,{code:t,userId:s},n)=>{const o=await g(e,n),a=await window.executeServerScript(e,{suiteletScriptId:"customscript"+l.SUITELET_SCRIPT_ID,deploymentId:"customdeploy"+l.SUITELET_SCRIPT_DEPLOY_ID,userId:s,code:t});return{success:a?.success!==!1&&!a?.error,logs:a?.logs||[],result:a?.result,error:a?.error,mutation:o}};window.renderFreemarkerTemplateServer=async(e,{template:t,recordType:s,recordId:n},o)=>{const a=await g(e,o),i=await window.executeFreemarkerTemplate(e,{suiteletScriptId:"customscript"+l.SUITELET_SCRIPT_ID,deploymentId:"customdeploy"+l.SUITELET_SCRIPT_DEPLOY_ID,deployUrl:a.deployUrl,template:t,recordType:s,recordId:n});return{success:!!i?.success,pdf:i?.pdf||"",mimeType:i?.mimeType||"application/pdf",error:i?.error,mutation:a}};window.deployMagicNetsuiteComponents=async(e,t,s)=>{const n=await window.checkMagicNetsuiteComponents(e),o=await g(e,s),a=await window.checkMagicNetsuiteComponents(e);return{success:!a?.error&&a?.allReady===!0,before:n,after:a,mutation:o}};async function g(e,t){const{query:s}=e,{folderExists:n,serverFileExists:o,mcpServerFileExists:a,suiteletScriptExists:i,suiteletDeployed:d}=await window.checkMagicNetsuiteComponents({query:s}),r={};if(n){const c=await T(e);r.folderId=c?.id}else try{const{folderId:c}=await window.createFolder(e,{folderName:l.FOLDER_NAME,parentFolderId:-15,csrfToken:t});r.folderId=c,await h(e,r.folderId)}catch(c){console.warn("[DEPLOY_SERVER_COMPONENTS] Folder creation failed; checking whether NetSuite created or already has the folder.",c);const m=await F(e).catch(()=>null);if(!m?.id)throw c;r.folderId=m.id,r.folderCreationRecovered=!0}if(!r.folderId)throw new Error("Magic Netsuite scripts folder is not available yet");const u=(await s.runSuiteQL.promise({query:"SELECT id, name FROM file WHERE folder = ? AND name IN (?, ?)",params:[String(r.folderId),l.SERVER_FILE,l.MCP_SERVER_FILE]})).asMappedResults(),f=new Map(u.map(c=>[c.name,c.id]));if(o)r.serverFileId=f.get(l.SERVER_FILE),await window.updateNetsuiteFileContent(e,{fileId:r.serverFileId,fileContent:S(),fileName:l.SERVER_FILE,folderId:r.folderId}),await E(e,r.folderId);else{const c=await window.uploadFile(e,{fileName:l.SERVER_FILE,fileContent:S(),folderId:r.folderId,csrfToken:t});r.serverFileId=c.uploaded?.[0]?.fileId;const m=await E(e,r.folderId);r.serverFileId=r.serverFileId||m?.id}if(!r.serverFileId)throw new Error("Magic Netsuite server file is not available yet");if(a)r.mcpServerFileId=f.get(l.MCP_SERVER_FILE),await window.updateNetsuiteFileContent(e,{fileId:r.mcpServerFileId,fileContent:v(),fileName:l.MCP_SERVER_FILE,folderId:r.folderId}),await I(e,r.folderId);else{const c=await window.uploadFile(e,{fileName:l.MCP_SERVER_FILE,fileContent:v(),folderId:r.folderId,csrfToken:t});r.mcpServerFileId=c.uploaded?.[0]?.fileId;const m=await I(e,r.folderId);r.mcpServerFileId=r.mcpServerFileId||m?.id}if(!r.mcpServerFileId)throw new Error("Magic Netsuite MCP server module file is not available yet");if(i){const[c]=(await s.runSuiteQL.promise({query:"SELECT id FROM script WHERE scriptid = ?",params:[l.scriptId]})).asMappedResults();r.scriptRecordId=c?.id}else{const{scriptRecordId:c}=await window.createScriptRecord(e,{name:l.SUITELET_SCRIPT_NAME,scriptId:l.SUITELET_SCRIPT_ID,fileId:r.serverFileId,scriptType:"SCRIPTLET",description:"Suitelet for Magic Netsuite extension server-side script execution",apiVersion:"2.1"},t);r.scriptRecordId=c;const m=await L(e);r.scriptRecordId=r.scriptRecordId||m?.id}if(!r.scriptRecordId)throw new Error("Magic Netsuite Suitelet script is not available yet");if(!d){const{deployUrl:c}=await window.createScriptDeployRecord(e,{name:l.SUITELET_DEPLOYMENT_NAME,scriptId:l.SUITELET_SCRIPT_DEPLOY_ID,scriptInternalId:r.scriptRecordId,title:l.SUITELET_DEPLOYMENT_NAME,status:"RELEASED",logLevel:"DEBUG"});r.deployUrl=c,await N(e,r.scriptRecordId)}return await R(e,r.deployUrl),r}const w=e=>new Promise(t=>setTimeout(t,e));function y(e,t){const{url:s}=e,n=s.resolveDomain({hostType:s.HostType.APPLICATION});return(a=>{if(!a)return"";let i=String(a).trim();return i=i.replace(/^https\/\//i,"https://"),i=i.replace(/^http\/\//i,"http://"),/^https?:\/\//i.test(i)?i:i.startsWith("//")?`https:${i}`:(i.startsWith("/")||(i=`/${i}`),`https://${n}${i}`)})(t||s.resolveScript({scriptId:"customscript"+l.SUITELET_SCRIPT_ID,deploymentId:"customdeploy"+l.SUITELET_SCRIPT_DEPLOY_ID,returnExternalUrl:!1}))}async function R(e,t){const s=y(e,t);let n="";for(let o=0;o<12;o++){try{const a=await fetch(s,{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded"},body:"action=pingMagicServer",credentials:"include"});if(a.ok){const i=await a.text(),d=JSON.parse(i);if(d?.success&&d?.ready)return;n=d?.error||i}else n=`HTTP ${a.status}`}catch(a){n=a?.message||String(a)}await w(750)}throw new Error(`Magic Netsuite server Suitelet is not ready yet: ${n}`)}async function p(e,t,s,n,o,a=1e4){const i=Date.now();let d="";for(;Date.now()-i<a;){try{const r=(await e.query.runSuiteQL.promise({query:t,params:s})).asMappedResults();if(n(r))return r}catch(r){d=r?.message||String(r)}await w(500)}throw new Error(`${o} did not settle before timeout${d?`: ${d}`:""}`)}async function _(e){await p(e,"SELECT id FROM script WHERE scriptid = ?",[l.scriptId],t=>t.length===0,"Suitelet script deletion")}async function h(e,t){const[s]=await p(e,"SELECT id FROM MediaItemFolder WHERE id = ?",[String(t)],n=>n.length>0,"Scripts folder creation");return s}async function T(e){return(await e.query.runSuiteQL.promise({query:`
        SELECT id, parent
        FROM MediaItemFolder
        WHERE name = ?
        ORDER BY CASE WHEN parent = -15 THEN 0 ELSE 1 END, id DESC
      `,params:[l.FOLDER_NAME]})).asMappedResults()[0]||null}async function F(e){const[t]=await p(e,`
      SELECT id, parent
      FROM MediaItemFolder
      WHERE name = ?
      ORDER BY CASE WHEN parent = -15 THEN 0 ELSE 1 END, id DESC
    `,[l.FOLDER_NAME],s=>s.length>0,"Scripts folder lookup");return t}async function E(e,t){const[s]=await p(e,"SELECT id FROM file WHERE folder = ? AND name = ?",[String(t),l.SERVER_FILE],n=>n.length>0,"Server file creation");return s}async function I(e,t){const[s]=await p(e,"SELECT id FROM file WHERE folder = ? AND name = ?",[String(t),l.MCP_SERVER_FILE],n=>n.length>0,"MCP server module file creation");return s}async function L(e){const[t]=await p(e,"SELECT id FROM script WHERE scriptid = ?",[l.scriptId],s=>s.length>0,"Suitelet script creation");return t}async function N(e,t){const[s]=await p(e,"SELECT primarykey AS id FROM scriptdeployment WHERE script = ?",[String(t)],n=>n.length>0,"Suitelet deployment creation");return s}async function b(e,t){await p(e,"SELECT id FROM file WHERE folder = ? AND name = ?",[String(t),l.SERVER_FILE],s=>s.length===0,"Server file deletion")}async function C(e,t){await p(e,"SELECT id FROM file WHERE folder = ? AND name = ?",[String(t),l.MCP_SERVER_FILE],s=>s.length===0,"MCP server module file deletion")}async function O(e,t){await p(e,"SELECT id FROM MediaItemFolder WHERE id = ?",[String(t)],s=>s.length===0,"Scripts folder deletion")}window.executeServerScript=async(e,{suiteletScriptId:t,deploymentId:s,userId:n,deployUrl:o,code:a})=>{const{url:i}=e;try{const d=y(e,o||i.resolveScript({scriptId:t,deploymentId:s,returnExternalUrl:!1})),r=await fetch(d,{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded"},body:`action=${encodeURIComponent(n)}&code=${encodeURIComponent(a)}`,credentials:"include"});if(!r.ok)throw new Error(`Server script failed: ${r.status}`);const u=await r.text();return console.log("[executeServerScript] Raw response:",u),JSON.parse(u)}catch(d){throw console.error("[executeServerScript]",d),d}};window.executeFreemarkerTemplate=async(e,{suiteletScriptId:t,deploymentId:s,deployUrl:n,template:o,recordType:a,recordId:i})=>{const{url:d}=e;try{const r=y(e,n),u=new URLSearchParams;u.set("action","renderFreemarker"),u.set("template",o),a&&i&&(u.set("recordType",a),u.set("recordId",i));const f=await fetch(r,{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded"},body:u.toString(),credentials:"include"});if(!f.ok)throw new Error(`FreeMarker render failed: ${f.status}`);const c=await f.text();return JSON.parse(c)}catch(r){throw console.error("[executeFreemarkerTemplate]",r),r}};window.removeMagicNetsuiteComponents=async(e,{},t)=>{const{query:s}=e,n=[],o=(i,d,r)=>{n.push(r!==void 0?{name:i,status:d,error:r}:{name:i,status:d})};try{const[{id:i}={}]=(await s.runSuiteQL.promise({query:"SELECT id FROM script WHERE scriptid = ?",params:[l.scriptId]})).asMappedResults();i?(await window.deleteNetsuiteScript(e,{scriptId:i,scriptName:"Magic Netsuite Server",apiVersion:"2.1",defaultFunction:"onRequest"},t),await _(e),o("Suitelet Script","removed")):o("Suitelet Script","skipped")}catch(i){o("Suitelet Script","error",String(i))}let a;try{const[{id:i}={}]=(await s.runSuiteQL.promise({query:"SELECT id FROM MediaItemFolder WHERE name = ? AND parent = -15",params:[l.FOLDER_NAME]})).asMappedResults();a=i}catch(i){return o("Folder Lookup","error",String(i)),{steps:n}}if(!a)return o("Server File","skipped"),o("Scripts Folder","skipped"),{steps:n};try{const i=(await s.runSuiteQL.promise({query:"SELECT id, name FROM file WHERE folder = ? AND name IN (?, ?)",params:[String(a),l.SERVER_FILE,l.MCP_SERVER_FILE]})).asMappedResults(),d=i.find(u=>u.name===l.SERVER_FILE)?.id,r=i.find(u=>u.name===l.MCP_SERVER_FILE)?.id;d?(await window.deleteNetsuiteFile(e,{fileId:d,folderId:a},t),await b(e,a),o("Server File","removed")):o("Server File","skipped"),r?(await window.deleteNetsuiteFile(e,{fileId:r,folderId:a},t),await C(e,a),o("MCP Server Module","removed")):o("MCP Server Module","skipped")}catch(i){o("Server Files","error",String(i))}try{await window.deleteFolder(e,{folderId:a}),await O(e,a),o("Scripts Folder","removed")}catch(i){o("Scripts Folder","error",String(i))}return{steps:n}};function v(){return`/**
 * @NApiVersion 2.1
 * @NModuleScope Public
 */
define(
